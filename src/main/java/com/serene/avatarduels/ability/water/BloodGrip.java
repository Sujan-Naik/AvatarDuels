...
public class BloodGrip extends BloodAbility implements AddonAbility, MultiAbility {
	...
	public BloodGrip(Player player, boolean sneak) {
		...
		if (!MultiAbilityManager.hasMultiAbilityBound(player, getName()) && sneak) {
			range = AvatarDuels.getConfig(getName()).getDouble("Abilities.Water.BloodGrip.Range");
			Entity e = GeneralMethods.getTargetedEntity(player, range);
		
			if (e == null || !(e instanceof LivingEntity)) {
				return;
			} else if (AvatarDuels.getConfig(getName()).getStringList("Abilities.Water.BloodGrip.EntityFilter").contains(e.getType().toString())) {
				return;
			}
			
			target = (LivingEntity) e;
			cooldown = AvatarDuels.getConfig(getName()).getLong("Abilities.Water.BloodGrip.Cooldown");
			dragSpeed = AvatarDuels.getConfig(getName()).getDouble("Abilities.Water.BloodGrip.DragSpeed");
			throwPower = AvatarDuels.getConfig(getName()).getDouble("Abilities.Water.BloodGrip.ThrowPower");
			mangleDmg = AvatarDuels.getConfig(getName()).getDouble("Abilities.Water.BloodGrip.MangleDamage");
			slamSpeed = AvatarDuels.getConfig(getName()).getDouble("Abilities.Water.BloodGrip.SlamSpeed");
			
			health = player.getHealth() - AvatarDuels.getConfig(getName()).getDouble("Abilities.Water.BloodGrip.DamageThreshold");
			TARGETS.add(target);
	...
	public boolean isEnabled() {
		//return AvatarDuels.getConfig(getName()).getBoolean("Abilities.Water.BloodGrip.Enabled");
		return false;
	}
