...
public class JCMethods {
	...
	public static void displayColoredParticles(String hex, Location location, int amount, double offsetX, double offsetY, double offsetZ, double extra, int alpha) {
		AvatarDuels.plugin.getParticleAdapter().displayColoredParticles(hex, location, amount, offsetX, offsetY, offsetZ, extra, alpha);
	}

	public static void emitLight(Location loc) {
		ConfigurationSection config = AvatarDuelsConfig.getConfig((Player)null);
	...
	public static void reload() {
		AvatarDuels.log.info("AvatarDuels Reloaded.");
		AvatarDuels.plugin.reloadConfig();
		AvatarDuels.logDebug = AvatarDuelsConfig.getConfig((World)null).getBoolean("Properties.LogDebug");
		AvatarDuelsConfig.board.reloadConfig();
		CoreAbility.registerPluginAbilities(AvatarDuels.plugin, "com.serene.avatarduels.ability");
		registerDisabledWorlds();
		registerCombos();
		RegenTempBlock.revertAll();
		BendingBoard.setFields();
		BendingBoard.updateOnline();
		AvatarDuels.plugin.initializeCollisions();
		FireTick.loadMethod();
