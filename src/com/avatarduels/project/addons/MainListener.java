...
import com.avatarduels.project.addons.ability.air.Deafen;
import com.avatarduels.project.addons.ability.air.FlightPassive;
import com.avatarduels.project.addons.ability.air.GaleGust;
import com.avatarduels.project.addons.ability.air.SonicWave;
import com.avatarduels.project.addons.ability.air.VocalMimicry;
import com.avatarduels.project.addons.ability.air.Zephyr;
import com.avatarduels.project.addons.ability.avatar.EnergyBeam;
import com.avatarduels.project.addons.ability.avatar.EnergyBeam.EnergyColor;
import com.avatarduels.project.addons.ability.chi.Dodging;
import com.avatarduels.project.addons.ability.chi.Jab;
import com.avatarduels.project.addons.ability.chi.Jab.JabHand;
import com.avatarduels.project.addons.ability.chi.NinjaStance;
import com.avatarduels.project.addons.ability.chi.WeakeningJab;
import com.avatarduels.project.addons.ability.earth.Accretion;
import com.avatarduels.project.addons.ability.earth.Bulwark;
import com.avatarduels.project.addons.ability.earth.Crumble;
import com.avatarduels.project.addons.ability.earth.Dig;
import com.avatarduels.project.addons.ability.earth.EarthKick;
import com.avatarduels.project.addons.ability.earth.LavaSurge;
import com.avatarduels.project.addons.ability.earth.MagmaSlap;
import com.avatarduels.project.addons.ability.earth.QuickWeld;
import com.avatarduels.project.addons.ability.earth.ShrapnelBlast;
import com.avatarduels.project.addons.ability.earth.ShrapnelShot;
import com.avatarduels.project.addons.ability.fire.ArcSpark;
import com.avatarduels.project.addons.ability.fire.ChargeBolt;
import com.avatarduels.project.addons.ability.fire.CombustBeam;
import com.avatarduels.project.addons.ability.fire.Electrify;
import com.avatarduels.project.addons.ability.fire.Explode;
import com.avatarduels.project.addons.ability.fire.FireDisc;
import com.avatarduels.project.addons.ability.fire.Jets;
import com.avatarduels.project.addons.ability.water.BloodGrip;
import com.avatarduels.project.addons.ability.water.MistShards;
import com.avatarduels.project.addons.ability.water.PlantArmor;
import com.avatarduels.project.addons.ability.water.RazorLeaf;
import com.avatarduels.project.addons.util.BendingPredicate;

public class MainListener implements Listener {
	
	private AvatarDuels plugin;
	private Map<Player, HashMap<Integer, String>> swapped;

	public MainListener(AvatarDuels plugin) {
		this.plugin = plugin;
	...
	public void onAbilityStart(AbilityStartEvent event) {
		if (BloodGrip.isBloodbent(event.getAbility().getPlayer())) {
			event.setCancelled(!AvatarDuels.instance.getConfig().getStringList("Abilities.Water.BloodGrip.BasicAbilities").contains(event.getAbility().getName()));
			...
			event.setCancelled(AvatarDuels.instance.getConfig().getStringList("Passives.Air.Flying.AbilityBlacklist").contains(event.getAbility().getName()));
		}
	}
	...
	public void onChat(AsyncPlayerChatEvent event) {
		...
		if (args[0].equalsIgnoreCase("@energycolor")) {
			if (!player.hasPermission("bending.ability.energybeam")) {
				player.sendMessage(AvatarDuels.instance.prefix() + ChatColor.RED + " You do not have permission to change color");
			} else if (args.length != 2) {
				player.sendMessage(AvatarDuels.instance.prefix() + ChatColor.RED + " Invalid format, try `@energycolor <color>`");
			} else {
				EnergyColor color = EnergyColor.valueOf(args[1].toUpperCase());
				
				if (color != null) {
					EnergyBeam.colors.put(player.getUniqueId(), color);
					player.sendMessage(AvatarDuels.instance.prefix() + ChatColor.GREEN + " Successfully set EnergyBeam color to " + args[1].toLowerCase());
				} else {
					player.sendMessage(AvatarDuels.instance.prefix() + ChatColor.RED + " Unknown color! Try red, blue, yellow, green, purple, orange, indigo, brown, white, or black!");
				}
			}
			event.setCancelled(true);
		} else if (args[0].equalsIgnoreCase("@vocalsound")) {
			if (!player.hasPermission("bending.ability.VocalMimicry")) {
				player.sendMessage(AvatarDuels.instance.prefix() + ChatColor.RED + " You do not have permission to change vocal noise");
			} else if (args.length != 2) {
				player.sendMessage(AvatarDuels.instance.prefix() + ChatColor.RED + " Invalid format, try `@vocalsound <sound>`");
			} else {
				try {
					...
					if (plugin.getConfig().getStringList("Abilities.Air.VocalMimicry.SoundBlacklist").contains(sound.toString())) {
						player.sendMessage(AvatarDuels.instance.prefix() + ChatColor.RED + " Cannot use that sound!");
					} else {
						VocalMimicry.selectSound(player, sound);
						player.sendMessage(AvatarDuels.instance.prefix() + ChatColor.GREEN + " Successfully set vocal sound to " + args[1].toLowerCase());
					}
				} catch (IllegalArgumentException e) {
					player.sendMessage(AvatarDuels.instance.prefix() + ChatColor.RED + " Unknown sound!");
				}
			}
			event.setCancelled(true);
		}
	...
	public void onReload(BendingReloadEvent event) {
		...
		new BukkitRunnable() {
			@Override
			public void run() {
				CoreAbility.registerPluginAbilities(plugin, "com.avatarduels.project.addons.ability");
			}
